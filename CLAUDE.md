...
- 新增员工（2026-09-10 先例：杨星）：**没有前端入口**，唯一路径是在 `config/employees.yaml` 的 `employees` 列表按文件末尾模板追加一条（`name_cn` / `name_en` / `type` / `standard_hours` / `department` / `position` / `responsibilities` / `onboard_date`）。`name_en` 是 Notion 成员名 → 中文名的映射键（`preprocessor.py:_standardize_member_names` 小写去空格后匹配），**必须与 Notion 账号名完全一致**，Notion 账号本身是中文时 `name_en` 直接填中文即可。填 `onboard_date` 后入职周之前不会因零工时误标偏低。加人不需要改代码：前端「预生成报告」实时重算、重启即生效；md/sidecar/PDF/Gamma 静态产物需重生。请假登记 + 新增员工同属 yaml 配置数据，可放同一分支一个 PR 合并，**只向 main 推一次**（多次连推会触发云端脏挂载 ImportError）；合并后打开应用「🏖️ 请假登记」下拉框看到新人即部署完成
- 请假登记（Claude Code 手动路径）：不走前端时直接改 `config/employees.yaml` 该员工的 `leaves` 列表（`start` / `end` / `type` / 可选 `note`，单日请假 start=end），格式与前端 `add_leave_to_yaml_text` 写入的一致。⚠️ 验证请假扣减时注意 `analyzer.py:analyze_members` 的 `date_range` 取的是**本周实际有数据的最小/最大日期**，不是整周——构造测试数据只到周三、请假在周五，会看到请假天数 0、标准工时未扣，这是既有行为不是配置错误；真实周报只要周五有人填工时就会计入。快速验证：构造含请假日的 DataFrame 跑 `preprocess_data(df, date)` → `TimesheetAnalyzer(df).analyze_members()`，看 `leave_days` / `standard_hours`
